// Omarchy KeyboardPanel, with centred placement for a standalone launcher.
import QtQuick
import Quickshell
...
PanelWindow {
  ...
  readonly property point cardOrigin: {
    if (!anchorItem || !bar) return Qt.point(Math.max(margin, (screenW - contentWidth) / 2), Math.max(margin, (screenH - contentHeight) / 2))
    var x = 0, y = 0
